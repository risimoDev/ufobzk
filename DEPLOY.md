...
**Решение** — выпускать эти домены через выход, который Google не метит как RU.
Поддерживаются три, приоритет сверху вниз:

1. **GEO-EXIT — своя нода.** Цепочка `главный → нода` по VLESS+REALITY, тем же
   механизмом, что каскад `RU-PROXY`. Нужна вторая машина.
2. **GEO-V6 — IPv6 главного сервера.** Ничего внешнего не требует: Google метит
   как RU только IPv4, а тот же сервер по IPv6 определяется как PL.
3. **WARP — Cloudflare.** Ненадёжен на серверных адресах, см. предупреждение ниже.

### GEO-V6: гео-домены через IPv6 главного (самый простой вариант)

Включается `GEO_IPV6=1` в `.env` и требует IPv6 внутри docker-сети `backend` —
в `docker-compose.yml` для неё уже стоит `enable_ipv6: true`. Без этого
freedom-аутбаунд падает с `network is unreachable`, ровно как это было с DNS у
WARP.

> **В `GEO_DOMAINS` при этом режиме можно класть только домены с AAAA-записью.**
> Домен без AAAA при `domainStrategy: UseIPv6` не резолвится и соединение
> обрывается. Проверено: у `antigravity.google` и `codeium.com` AAAA нет, у
> `gemini.google.com`, `cloudcode-pa`, `cloudaicompanion` и
> `generativelanguage.googleapis.com` — есть. Поэтому дефолтный список для
> IPv6-режима свой и содержит только безопасные домены; проще оставить
> `GEO_DOMAINS` пустым.

Проверить AAAA перед добавлением домена:
...
> и лечению параметрами (`reserved`, `MTU`, endpoint, DNS) не поддаётся. Если
> `scripts/15-diagnose-warp.sh` показывает такую картину на всех вариантах —
> переходите на GEO-EXIT, а не подбирайте настройки.

### GEO-EXIT: настройка

Сначала выберите ноду — ту, у которой YouTube отдаёт не `RU`. Проверить можно
прямо на ноде:

```bash
curl -s -4 -H 'User-Agent: Mozilla/5.0' https://www.youtube.com/ | grep -o '"INNERTUBE_CONTEXT_GL":".."'
```

Затем на главном в `.env`:

```bash
GEO_NODE_NAME=nl-node-2          # поле name из таблицы servers (как в админке)
GEO_TRANSIT_UUID=<случайный uuid>  # python3 -c 'import uuid; print(uuid.uuid4())'
```

`host`, `reality_port`, `reality_public_key`, `reality_short_id` и SNI берутся из
БД по имени — дублировать их в `.env` не нужно, и при смене ключей ноды ничего
править не придётся.

`GEO_TRANSIT_UUID` автоматически добавляется в клиенты REALITY-инбаунда **всех**
нод (`app/remote_xray.py`), поэтому переключить выход можно одним
`GEO_NODE_NAME`. Порядок применения важен — сначала ноды, потом главный:

```bash
# 1. Прописать обе переменные в .env, затем пересоздать приложение
docker compose up -d --force-recreate --no-deps ufo-app

# 2. Синхронизировать ноды из админки (Servers → Sync), чтобы транзитный
#    клиент попал в их конфиги. Без этого нода отобьёт цепочку.

# 3. Проверить, что трафик пошёл верно
docker exec ufobzk-xray tail -f /var/log/xray/access.log | grep --line-buffered -E 'youtube|gemini'
```

В логе должно быть `-> GEO-EXIT`. Если `GEO_NODE_NAME` не найден среди активных
серверов или у ноды не заполнены ключи REALITY, GEO-EXIT молча выключается, а
причина пишется в лог приложения — смотрите `docker logs ufobzk-app`.
