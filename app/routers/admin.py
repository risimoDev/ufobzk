...
# ── Servers (remote Xray nodes) ──

from app.remote_xray import push_config_to_server, sync_all_servers


@router.get("/admin/api/servers")
async def api_servers_list(admin: User = Depends(require_admin), db: Session = Depends(get_db)):
    """Список всех remote Xray серверов."""
    servers = db.query(Server).order_by(Server.priority, Server.id).all()
    return JSONResponse([{
        "id": s.id,
        "name": s.name,
        "host": s.host,
...
@router.post("/admin/api/servers")
async def api_server_create(
    request: Request,
    admin: User = Depends(require_admin),
    db: Session = Depends(get_db),
):
    """Добавить новый remote Xray сервер."""
    ...
    s = Server(
        ...
        region=data.get("region", "eu").strip(),
        role=data.get("role", "proxy").strip(),
        ws_port=int(data.get("ws_port", 443)),
        reality_port=int(data.get("reality_port", 2053)),
        grpc_port=int(data.get("grpc_port", 8445)),
        xhttp_port=int(data.get("xhttp_port", 8444)),
        priority=int(data.get("priority", 100)),
        reality_public_key=data.get("reality_public_key", "").strip() or None,
        reality_private_key=data.get("reality_private_key", "").strip() or None,
        reality_dest=data.get("reality_dest", "").strip() or None,
        reality_server_names=data.get("reality_server_names", "").strip() or None,
        reality_short_id=data.get("reality_short_id", "").strip() or None,
    )
    db.add(s)
    db.commit()
    _log_action(db, admin.id, "server_create", s.name, f"region={s.region} host={s.host}")
    return JSONResponse({"ok": True, "id": s.id})


@router.post("/admin/api/servers/{server_id}/delete")
async def api_server_delete(
    server_id: int,
    admin: User = Depends(require_admin),
    db: Session = Depends(get_db),
):
    """Удалить remote сервер."""
    ...
    db.delete(s)
    db.commit()
    _log_action(db, admin.id, "server_delete", name)
    return JSONResponse({"ok": True})


@router.post("/admin/api/servers/{server_id}/sync")
async def api_server_sync(
    server_id: int,
    admin: User = Depends(require_admin),
    db: Session = Depends(get_db),
):
    """Ручная синхронизация конфига на один remote сервер."""
    s = db.query(Server).filter(Server.id == server_id).first()
    if not s:
        raise HTTPException(status_code=404, detail="Server not found")
    keys = db.query(VPNKey).filter(VPNKey.is_active == True).all()  # noqa: E712
    success = await push_config_to_server(s, keys)
    _log_action(db, admin.id, "server_sync", s.name, "ok" if success else "failed")
    return JSONResponse({"ok": success})


@router.post("/admin/api/servers/sync-all")
async def api_servers_sync_all(
    admin: User = Depends(require_admin),
    db: Session = Depends(get_db),
):
    """Ручная синхронизация на все active серверы."""
    results = await sync_all_servers(db)
    ok_count = sum(1 for v in results.values() if v)
    _log_action(db, admin.id, "servers_sync_all", f"{ok_count}/{len(results)} OK")
    return JSONResponse({"ok": True, "results": results})
